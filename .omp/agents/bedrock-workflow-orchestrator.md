...
Derive the head-prefix segment mechanically from the exact trigger SHA; never
type, estimate, or shorten it manually.
...
- A pushed checkpoint or stage commit is immutable. Never amend, rebase,
  reset, or force-push it. Repair malformed metadata with a new superseding
  stage commit and disclose the repair in run evidence.
