...
When records cite a donor that README rewriting will change, cite the opening
tree as `<trigger-commit>:<repository-relative-path>`, not the future live path.
Review every root `README*.md`; keep localized versions aligned on purpose,
status, fork topology, and situation pointers.

Also inspect any write-privileged automation introduced by the triggering PR.
If it remains in the operational tree, record its authority boundary, admission,
and immutable-protocol behavior proportionally rather than treating it as
invisible setup.
...
Before pushing, parse the commit body and verify every required trailer. After
push, the commit is immutable: never amend/rebase/reset/force-push it. Repair a
malformed stage with a new superseding stage commit and report both SHAs.
...
Update all root `README*.md` orientations together when their shared claims or
fork instructions change.
